...
class TestQuaternionMethods(unittest.TestCase):
    ...
    def test_rad_to_deg(self):
        self.assertEqual(my_quaternion.rad_to_deg(my_quaternion.math.pi), 180.0)

    def test_quat_to_euler(self):
        quaternion = [my_quaternion.math.cos(math.pi/2.0), 0.0, 0.0, -my_quaternion.math.sin(math.pi/4.0)]
        euler = my_quaternion.quat_to_euler(quaternion)
    ...
    def test_normalize_quat(self):
        ...
        quaternion = my_quaternion.normalize_quat(quaternion)
    ...
    def test_inv_quat(self):
        quaternion = [ my_quaternion.math.cos(math.pi/4.0), 0.0, 0.0, -my_quaternion.math.sin(math.pi/4.0)]
        expectation = [ -my_quaternion.math.cos(math.pi/4.0), 0.0, 0.0, -my_quaternion.math.sin(math.pi/4.0)]
        self.assertListEqual( my_quaternion.inv_quat(quaternion), expectation)
    ...
    def test_multiplication_quat(self):
        ...
        result = my_quaternion.multiplication_quat(quaternion1, quaternion2)
    ...
    def test_get_changed_orientation(self):
        ...
        result = my_quaternion.get_changed_orientation(quaternion2, quaternion1)
        result = list(map(lambda x: round(x, 4), result))
        self.assertListEqual(result, expectation)
